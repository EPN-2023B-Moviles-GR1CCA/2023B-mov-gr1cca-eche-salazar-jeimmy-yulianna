...
class BDMemoria (contexto: Context):SQLiteOpenHelper(contexto, BD, null, 1){
    override fun onCreate(db: SQLiteDatabase?) {
        var sql="CREATE TABLE artistas (idArtista INTEGER PRIMARY KEY AUTOINCREMENT, "+
                "nombre VARCHAR(250), " +
    ...
    override fun onUpgrade(db: SQLiteDatabase?, oldVersion: Int, newVersion: Int) {
        val ordenB= "DROP TABLE IF EXISTS artistas"
        db!!.execSQL(ordenB)
        onCreate(db)
    ...
    fun crearArtista(artista:Artista): String {
        ...
        var resultado=db.insert("artistas", null, contenedor)
